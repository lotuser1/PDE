...
def feature_extract(cfg, model, data_loader):
    ...
    with EventStorage() as storage:
        for iter_id in range(epoch_size):
            ...
            if iter_id > 0 and iter_id * batch_size % 3000 == 0:
                file_batch_id += 1
                torch.save(all_box_features, 'features/'
                                             f'30shot_gt/all_box_features_batch{file_batch_id}.pt')
                torch.save(all_box_labels, 'features/'
                                           f'30shot_gt/all_box_labels_batch{file_batch_id}.pt')
...
if __name__ == "__main__":
    ...
    args.num_gpus = 1
    args.config_file = 'configs/COCO-detection/finetune_pseudo_30shot.yaml'
