...
# from fsdet.modeling.roi_heads_mb import build_roi_heads
...
class GeneralizedRCNN(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if cfg.MODEL.ROI_HEADS.FREEZE_FEAT:
            for p in self.roi_heads.box_head.parameters():
                p.requires_grad = False
            # for p in self.roi_heads.box_predictor.bbox_pred.parameters():
            #     p.requires_grad = False
    ...
    def extract_features(self):
        ...
        for iter_id in range(epoch_size):
            ...
            box_features, box_labels = self.roi_heads(
                images, features, proposals, gt_instances, extractor=True
            )
            box_features_split = box_features.view(self.cfg.SOLVER.IMS_PER_BATCH, -1, 1024).to('cpu')
        ...
        torch.save(all_box_features, 'all_box_features.pt')
        torch.save(all_box_labels, 'all_box_labels.pt')


@META_ARCH_REGISTRY.register()
class FeatureGeneralizedRCNN(nn.Module):
    """
    Generalized R-CNN. Any models that contains the following three components:
    1. Per-image feature extraction (aka backbone)
    2. Region proposal generation
    ...
    def forward(self, batched_inputs):
        ...
        if not self.training:
            return self.inference(batched_inputs)
        box_features, proposals = batched_inputs
        batch_size = box_features.size(0)
        box_features = box_features.view(-1, 1024).to('cuda')
        proposals_instances = []
        for img_id in range(batch_size):
            img_size = tuple(proposals['image_size'][img_id].tolist())

            pro_inst = Instances(img_size)
            proposal_boxes = Boxes(proposals['proposal_boxes'][img_id])

            pro_inst.set('proposal_boxes', proposal_boxes)
            pro_inst.set('objectness_logits', proposals['proposal_boxes'][img_id])
            pro_inst.set('gt_classes', proposals['gt_classes'][img_id])
            pro_inst.set('is_pseudo', proposals['is_pseudo'][img_id])
            gt_boxes = Boxes(proposals['gt_boxes'][img_id])
            pro_inst.set('gt_boxes', gt_boxes)
            proposals_instances.append(pro_inst.to('cuda'))

        _, detector_losses = self.roi_heads.feature_train(box_features, proposals_instances)
