...
def inference_on_dataset(model, data_loader, evaluator):
    ...
    vis = True
    if vis:
        anno_path = '/media/hz/dataset/lizi/datasets/cocosplit/datasplit/trainvalno5k.json'
    ...
    with inference_context(model), torch.no_grad():
        for idx, inputs in enumerate(data_loader):
            ...
            if vis:
                ...
                keep_thres = scores > 0.5  # 可视化时候的阈值
...
def pseudo_on_dataset(model, data_loader, evaluator):
    ...
    with inference_context(model), torch.no_grad():
        ...
        coco_path = 'pseudo_30shot_thres06.json'
...
def find_pseudo(inputs, outputs, id_map, base_classes, novel_classes, trainval_coco, thres=0.6):
    ...
    pred_boxes = prediction.get('pred_boxes')
    #x0y0x1y1--->xywh
    convert_pred_boxes = pred_boxes.clone()  # 初始化
    for i, box in enumerate(convert_pred_boxes):
        x = (box[0] + box[2]) / 2
        y = (box[1] + box[3]) / 2
        w = box[2] - box[0]
        h = box[3] - box[1]
        pred_boxes.tensor[i, :] = torch.FloatTensor([x, y, w, h])
