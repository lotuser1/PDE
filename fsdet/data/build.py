...
class FeatureDataset(torch.utils.data.Dataset):  # 创类：MyDataset,继承torch.utils.data.Dataset
    def __init__(self, dataset_batch=1):
        ...
        root_path = 'features/10shot_thres06'
        feature_name = f'all_box_features_batch{dataset_batch}.pt'
        label_name = f'all_box_labels_batch{dataset_batch}.pt'
        # root_path = f'10shot_thres06_cluster/batch{dataset_batch}'
        # feature_name = f'batch1_fg_feature/batch1_class47_fea.pt'
        # label_name = f'batch1_fg_label/batch1_class47_label.pt'
        print(f'load feature dataset batch: {os.path.join(root_path, feature_name)}')
