...
def load_coco_json(json_file, image_root, metadata, dataset_name):
    ...
    if is_shots:
        fileids = {}
        split_dir = os.path.join("/media/hz/dataset/lizi/datasets", "cocosplit")
    ...
    if is_shots:
        ...
        for (img_dict, anno_dict_list) in imgs_anns:
            ...
            for anno in anno_dict_list:
                ...
                # 当该对象类别属于base classes时，才会添加该标记
                if obj["category_id"] in id_map:
                    if not is_test:
                        # base_class_num[id_map[obj["category_id"]]] += 1
                        # if base_class_num[id_map[obj["category_id"]]] > 1000:
                        #     continue
                        if obj["category_id"] in base_id_map:
                            base_class_num[base_id_map[obj["category_id"]]] += 1
                            if base_class_num[base_id_map[obj["category_id"]]] > 300:
                                continue
                    obj["category_id"] = id_map[obj["category_id"]]

                    objs.append(obj)
            record['is_pseudo'] = True
...
def load_coco_json_generate(json_file, image_root, metadata, dataset_name):
    """
    Load a json file with COCO's instances annotation format.
    Currently supports instance detection.
    Args:
...
def register_meta_coco(name, metadata, imgdir, annofile):
    if 'generate' in name:
        DatasetCatalog.register(
            name,
            lambda: load_coco_json_generate(annofile, imgdir, metadata, name),
        )
    # elif 'coco_trainval_pseudo' in name:
    #     DatasetCatalog.register(
    #         name,
    #         lambda: load_coco_json_pseudo(annofile, imgdir, metadata, name),
    #     )
    else:
        DatasetCatalog.register(
            name,
